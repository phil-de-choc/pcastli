...
/* Verifying if the root is in g_lst, adding it if not. */
void find_add_root(node* rootpar)
{
   ...
   while (i_pt)
   {
      if (i_pt->pt == PT_NODE)
      {
         if (i_pt->aptr.ptr == rootpar) return;
      }

      i_pt = i_pt->next;
   }

   g_lst_add(rootpar, PT_NODE);
}



/* Of all the nodes belonging to the same tree in g_lst, this function removes 
   all except the root. */
void keep_roots(void)
{
   ...
   while (i_pt)
   {
      ...
      {
         /* to get the root */
         node* parent = i_pt->aptr.ptr;
         while (parent->parent)
         {
            parent = parent->parent;
         }

         if (parent != i_pt->aptr.ptr)
         {
            g_item* i_pt_next = NULL;

            /* verifying if the root is in g_lst, adding it if not */
            find_add_root(parent);

            /* removing the actual g_item */
...
void gc(void)
{
   ...
   while (i_pt)
   {
      ...
      if (i_pt->pt == PT_NODE)
      {
         ...
         if (!stack_top)
            fatal_error("Error: Lack of memory in gc for stack_top.");
         ...
         while (stack_top && !found)
         {
            ...
            for (i_child = 0; i_child < searched_aptr.ptr->nb_children; i_child++)
            {
               ...
               if (!new_top)
                  fatal_error("Error: Lack of memory in gc for new_top.");
...
void g_lst_add(void* vptr, pointer_type pt)
{
   ...
   if (!g_lst)
   {
      ...
      if (!g_lst)
         fatal_error("Error: Lack of memory in g_lst_add for g_lst.");
   ...
   else
   {
      ...
      if (!last->next)
         fatal_error("Error: Lack of memory in g_lst_add for last->next.");
