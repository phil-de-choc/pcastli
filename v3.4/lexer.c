...
int token(void)
{
   ...
   if (c == '0')
   {
      ...
      if (c2 == 'x')
      {
         ...
         yylval = malloc(sizeof(node));
         if (!yylval)
            fatal_error("Error: Lack of memory in token for a number.");
         memset(yylval, 0, sizeof(node));
   ...
   if (c == '\"')
   {
      ...
      if(!symbuf)
         fatal_error("Error: Lack of memory for symbuf in token.");
      ...
      while (c != '\"')
      {
         ...
         if (i == length - 1)
         {
            ...
            if(!symbuf)
               fatal_error("Error: Lack of memory by realloc\n"
                  "for symbuf in token.");
         }
...
escape_ok:
      ...
      yylval = malloc(sizeof(node));
      if (!yylval)
         fatal_error("Error: Lack of memory in token for a string node.");
      memset(yylval, 0, sizeof(node));
      ...
      if (!yylval->opval.str.tab)
         fatal_error("Error: Lack of memory in token for a string.\n");
   ...
   if (isalpha(c) || c == '_')
   {
      ...
      while (reserved_words_table[i].rword)
      {
         if (!strcmp(symbuf, reserved_words_table[i].rword))
         {
               ...
               reserved_words_table[i].num_symbol == STDERR)
            {
               yylval = malloc(sizeof(node));
               if (!yylval)
                  fatal_error("Error: Lack of memory for reserved "
                     "word node in token.\n");
               memset(yylval, 0, sizeof(node));
               if (reserved_words_table[i].num_symbol == PARENT)
      ...
      yylval = malloc(sizeof(node));
      if (!yylval)
         fatal_error("Error: Lack of memory in token for a variable.");
      memset(yylval, 0, sizeof(node));
      yylval->ntype = NT_VARIABLE;
      yylval->opval.name = malloc(strlen(symbuf) + 1);
      if (!yylval->opval.name)
         fatal_error("Error: Lack of memory in token "
            "for a variable name.\n");
   ...
   case '=':
      ...
      yylval = malloc(sizeof(node));
      if (!yylval)
         fatal_error("Error: Lack of memory in yylex for \'=\'.");
      memset(yylval, 0, sizeof(node));
      yylval->ntype = NT_MATH_OPER;
...
void ifpush(void)
{
   if(contextp - contextstack >= CONTEXTSTACK_SIZE)
      fatal_error("Error: Contextstack overflow.");
   *++contextp = 'i';
}
